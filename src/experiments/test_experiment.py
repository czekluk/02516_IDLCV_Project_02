...
def test_experiment():
    ...
    if args.models in ["encdec", "both"]:
        for dm_instance in [dm_drive, dm]:
            ...
            if args.dataset in [dataset_type, "both"]:
                for criterion in criterion_functions:
                    ...
                    epochs_list = [get_epochs(dataset_type, model.__class__.__name__) for model in enc_dec_models]
                    trainer = Trainer(
    ...
    if args.models in ["unet", "both"]:
        for dm_instance in [dm_drive, dm]:
            ...
            if args.dataset in [dataset_type, "both"]:
                for criterion in criterion_functions:
                    optimizers = deepcopy(optimizers_gl)
                    epochs_list = [get_epochs(dataset_type, model.__class__.__name__) for model in unet_models]
                    trainer = Trainer(
